...
<div  class="">
    ...
    <div id="professional" class="hidden">

        @foreach($employees as $index=>$employee)
        <div wire:key="{{$index}}" id="professional-{{$index}}" class="flex flex-row p-6 text-sm card " style="width: 20rem;">
          <div class="basis-1/2">
            <img onclick="openModal('modalConfirm', '{{$index}}')" class="card-img-top" src="images/{{$employee['employee_id']}}.jpeg" alt="Foto {{$employee['employee_name']}}">
            ...
            <div class="text-sm text-black">Escolha um ou mais serviços abaixo:</div>
                ...
                @foreach($employee['service'] as $service)
                    <div class="p-1 text-sm text-black ">
                        <input id="service" wire:model="chosen_services"
              ...
              <a href="#" onclick="closeModal('modalConfirm')"
    ...
    <div id="confirmation" class="">
        ...
        @if($choosedEmployee !== null && empty($chosen_services))
            <div class="p-2">
                O funcionário escolhido é: {{$employees[$choosedEmployee]['employee_name']}}
            </div>
            <div class="p-2">
        ...
        @endif
         @if($choosedEmployee !== null && !empty($chosen_services) && count($chosen_services)>1)
             <div class="p-2">
                 O funcionário escolhido é: <div class="font-bold">{{$employees[$choosedEmployee]['employee_name']}}</div>
             </div>
            <div class="p-2">
            Os serviços selecionados são:<br>
                @foreach($chosen_services as $item=>$service)
                   - {{$service}} <button class="text-white  bg-gray-500 button rounded">Alterar</button><br>
                @endforeach
            </div>
        @endif
            @if($choosedEmployee !== null || !empty($chosen_services))
        <button class="rounded bg-green-300 text-white">Alterar </button>
            @endif
