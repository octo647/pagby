...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    {{-- Total de Recompensas Ativas --}}
    <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-lg p-6 text-white">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-blue-100 text-sm font-medium">Recompensas Ativas</p>
                <p class="text-3xl font-bold mt-2">{{ $estatisticas['total_recompensas_ativas'] }}</p>
            </div>
    ...
    <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl shadow-lg p-6 text-white">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold mt-2">R$ {{ number_format($estatisticas['valor_total_creditos'], 2, ',', '.') }}</p>
            </div>
    ...
    <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl shadow-lg p-6 text-white">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold mt-2">{{ $estatisticas['vendas_produtos_periodo'] }}</p>
                <p class="text-purple-100 text-xs mt-1">Últimos {{ $periodo }} dias</p>
            </div>
    ...
    <div class="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl shadow-lg p-6 text-white">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold mt-2">{{ $estatisticas['clientes_ativos'] }}</p>
...
{{-- Gráfico/Informações Adicionais --}}
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    {{-- Recompensas Usadas no Período --}}
    <div class="bg-white rounded-xl shadow-md p-6">
        ...
        <div class="space-y-4">
            <div class="flex justify-between items-center p-4 bg-gray-50 rounded-lg">
                ...
                <span class="text-2xl font-bold text-indigo-600">{{ $estatisticas['recompensas_usadas_periodo'] }}</span>
            </div>
            @if($estatisticas['total_recompensas_ativas'] > 0)
                <div class="flex justify-between items-center p-4 bg-gray-50 rounded-lg">
                    <span class="text-sm font-medium text-gray-700">Taxa de Conversão</span>
                    <span class="text-2xl font-bold text-green-600">
                        {{ number_format(($estatisticas['recompensas_usadas_periodo'] / ($estatisticas['recompensas_usadas_periodo'] + $estatisticas['total_recompensas_ativas'])) * 100, 1) }}%
                    </span>
                </div>
            @endif
        </div>
