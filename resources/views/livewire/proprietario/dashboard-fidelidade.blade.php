<div class="p-6 space-y-6">
    ...
    <div class="border-b border-gray-200">
        <nav class="-mb-px flex space-x-8">
            <button wire:click="$set('abaSelecionada', 'visao-geral')" 
                    class="@if($abaSelecionada === 'visao-geral') border-indigo-500 text-indigo-600 @else border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 @endif whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition">
                📊 Visão Geral
            </button>
            <button wire:click="$set('abaSelecionada', 'vinculos')" 
                    class="@if($abaSelecionada === 'vinculos') border-indigo-500 text-indigo-600 @else border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 @endif whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition">
                🔗 Produtos & Serviços
            </button>
            <button wire:click="$set('abaSelecionada', 'recompensas')" 
                    class="@if($abaSelecionada === 'recompensas') border-indigo-500 text-indigo-600 @else border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 @endif whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition">
                🎁 Recompensas Ativas
            </button>
            <button wire:click="$set('abaSelecionada', 'produtos')" 
                    class="@if($abaSelecionada === 'produtos') border-indigo-500 text-indigo-600 @else border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 @endif whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition">
    ...
    {{-- Conteúdo das abas --}}
    <div class="mt-6">
        @if($abaSelecionada === 'visao-geral')
            @include('livewire.proprietario.fidelidade.visao-geral')
        @elseif($abaSelecionada === 'vinculos')
            @include('livewire.proprietario.fidelidade.vinculos')
        @elseif($abaSelecionada === 'recompensas')
            @include('livewire.proprietario.fidelidade.recompensas')
        @elseif($abaSelecionada === 'produtos')
            @include('livewire.proprietario.fidelidade.produtos')
        @endif
    </div>
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <select wire:model="serviceId" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    ...
                                    @foreach($servicos as $servico)
                                        <option value="{{ $servico->id }}">{{ $servico->service }} - R$ {{ number_format($servico->price, 2, ',', '.') }}</option>
                                    @endforeach
                                </select>
                                @error('serviceId') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <select wire:model="estoqueId" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    ...
                                    @foreach($produtos as $produto)
                                        <option value="{{ $produto->id }}">{{ $produto->produto_nome }} - R$ {{ number_format($produto->preco_unitario, 2, ',', '.') }}</option>
                                    @endforeach
                                </select>
                                @error('estoqueId') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    @error('prioridade') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="number" wire:model="percentualDesconto" min="0" max="100" step="0.01" 
                                           class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    @error('percentualDesconto') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                </div>
                            </div>
