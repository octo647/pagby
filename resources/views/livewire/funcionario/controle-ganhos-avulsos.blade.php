    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 mb-8 flex flex-col md:flex-row gap-4 md:items-end">
            ...
            <div class="flex flex-col w-full">
                ...
                <input type="text" value="{{ number_format($comissao, 2, ',', '.') }}%" readonly class="border border-blue-300 rounded-lg px-3 py-2 bg-slate-100 text-blue-900 font-bold">
            </div>
        </div>
                    ...
                    <tbody class="bg-white divide-y divide-blue-50">
                        @foreach($servicos as $servico)
                            <tr>
                                ...
                                <td class="px-4 py-2">{{ $servico->descricao ?? $servico->servico_nome ?? '-' }}</td>
                                <td class="px-4 py-2">R$ {{ number_format($servico->valor, 2, ',', '.') }}</td>
                            </tr>
                        @endforeach
                        @foreach($produtos as $produto)
                            <tr>
                                ...
                                <td class="px-4 py-2">R$ {{ number_format($produto->valor, 2, ',', '.') }}</td>
                            </tr>
                        @endforeach
