...
<div class="min-h-screen bg-gray-50 py-6">
    ...
    <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-6">📅 Agenda de {{ $funcionario->name ?? Auth::user()->name }}</h1>
                ...
                <tbody>
                    @forelse($agendamentos as $agendamento)
                        ...
                        <tr class="hover:bg-gray-50 transition-colors duration-200">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">{{ $agendamento->services }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @endphp
                                @if($jaPassou && in_array($agendamento->status, ['Realizado', 'Cancelado']))
                                    <span class="inline-block px-2 py-1 rounded bg-green-100 text-gray-800 text-base font-medium">{{ $agendamento->status }}</span>
                                @elseif($hoje && $agendamento->status !== 'Cancelado')
                                    <div class="flex flex-col gap-2 items-start">
                                        <label class="inline-flex items-center">
                                            <input type="radio" name="status_{{$agendamento->id}}" value="Cancelado" wire:click="confirmarCancelamento({{$agendamento->id}})" class="form-radio text-red-600">
            ...
            @forelse($agendamentos as $agendamento)
                ...
                <div class="bg-white rounded-xl shadow p-4 flex flex-col gap-2">
                    <div class="flex items-center justify-between mb-2">
                        <div>
                            <span class="text-sm font-semibold text-gray-700">{{ \Carbon\Carbon::parse($agendamento->appointment_date)->format('d/m/Y') }}</span>
                            <span class="ml-2 text-xs text-gray-500">{{ \Carbon\Carbon::createFromFormat('H:i:s', $agendamento->start_time)->format('H:i') }} - {{\Carbon\Carbon::createFromFormat('H:i:s', $agendamento->end_time)->format('H:i') }}</span>
                        </div>
                    ...
                    @endphp
                    @if($jaPassou && in_array($agendamento->status, ['Realizado', 'Cancelado']))
                        <span class="inline-block px-2 py-1 rounded bg-gray-100 text-gray-800 text-xs font-medium">{{ $agendamento->status }}</span>
                    @elseif($hoje && $agendamento->status !== 'Cancelado')
