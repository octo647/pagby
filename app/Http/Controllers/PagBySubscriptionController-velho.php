...
class PagBySubscriptionController extends Controller
{
    ...
    public function showPaymentForm(Request $request)
    {
        ...
        if (strlen($phone) <= 9) {
            $phone = '(11) ' . $phone; // DDD padrão
        }
        
        $customerData = [
            'name' => $contact->owner_name ?? $contact->tenant_name,
            'email' => $contact->email,
            'cpfCnpj' => preg_replace('/\D/', '', $contact->cpf),
            'phone' => $phone,             
            'postalCode' => $contact->cep ?? null,
            'address' => $contact->address ?? null,
            'addressNumber' => $contact->address_number ?? '01',
            'complement' => $contact->complement ?? 'Casa',
            'province' => $contact->neighborhood ?? null, // bairro
        ...
        // Montar dados da assinatura recorrente para o Asaas; será criada uma assinatura recorrente mensal, trimestral, semestral ou anual conforme o plano escolhido
        ...
        $subscriptionData = [
            ...
            'billingType' => 'UNDEFINED', // ou BOLETO, CREDIT_CARD, etc, se quiser forçar
        ];
       
     

        $asaasService = new \App\Services\AsaasService();
        $asaasResult = $asaasService->criarAssinatura($customerData, $subscriptionData);
       
        \Log::info('Resultado criação assinatura Asaas:', $asaasResult);
        ...
        if ($asaasResult['success']) {
            ...
            $payment = PagByPayment::create([
                ...
                'type' => 'subscription',
                'plan' => $planName,
                'description' => 'Assinatura Asaas: ' . ($asaasResult['data']['invoiceUrl'] ?? '')
            ]);

            // Se não veio o invoiceUrl, buscar a cobrança da assinatura
            if (empty($asaasResult['data']['invoiceUrl']) && !empty($asaasResult['data']['id'])) {
                $asaasService = new \App\Services\AsaasService();
                $payments = $asaasService->listarCobrancasAssinatura($asaasResult['data']['id']);
                if (!empty($payments['data']) && is_array($payments['data']) && count($payments['data']) > 0) {
                    $firstPayment = $payments['data'][0];
                    if (!empty($firstPayment['invoiceUrl'])) {
                        $payment->description = 'Assinatura Asaas: ' . $firstPayment['invoiceUrl'];
                        $payment->asaas_payment_id = $firstPayment['id'] ?? null;
                        $payment->save();
                    }
                }
            }
            // Redireciona para a página de aguarde, onde o cliente poderá ver a fatura e pagar
            return redirect()->route('pagby-subscription.wait', ['paymentId' => $payment->id]);
        } else {
            $erro = $asaasResult['message'] ?? 'Erro ao criar assinatura recorrente no Asaas.';
            if (isset($asaasResult['errors'])) {
                $erro .= ' Detalhes: ' . (is_array($asaasResult['errors']) ? json_encode($asaasResult['errors']) : $asaasResult['errors']);
            }
    ...
    public function processPayment(Request $request)
    {
        ...
        \Log::info('Resultado criação checkout Asaas:', $asaasResult);
    ...
    public function asaasPay(Request $request, $paymentId)
    {
        ...
        $asaasService = new \App\Services\AsaasService();
        $result = $asaasService->criarAssinatura($customerData, $paymentData);
    ...
    if ($request->has('event') && $request->has('payment')) {
        ...
        if ($asaasPaymentId) {
            ...
            if ($payment) {
                ...
                // Busca adicional por subscription_id
                $asaasSubscriptionId = $request->input('payment.subscription');
                $paymentBySub = null;
                if ($asaasSubscriptionId) {
                    $paymentBySub = PagByPayment::where('asaas_subscription_id', $asaasSubscriptionId)->first();
                }
                if ($paymentBySub) {
                    Log::info('Pagamento encontrado por asaas_subscription_id', [
                        'asaas_subscription_id' => $asaasSubscriptionId,
                        'payment_id' => $paymentBySub->id
                    ]);
                    $oldStatus = $paymentBySub->status;
                    $paymentBySub->status = $asaasStatus;
                    $paymentBySub->asaas_payment_id = $asaasPaymentId;
                    $paymentBySub->save();
                } else {
                    Log::warning('Pagamento não encontrado para asaas_payment_id, paymentLink ou asaas_subscription_id', [
                        'asaas_payment_id' => $asaasPaymentId,
