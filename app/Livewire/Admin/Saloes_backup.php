...
class Saloes extends Component
{
   ...
   /**
    ...
    private function createTenantHomeView($tenantId, $tenantType)
    {
        ...
        // Template baseado no tipo do estabelecimento (case-insensitive)
        if (strtolower($tenantType) === 'barbearia') {
...
{
        $templatePath = resource_path('Templates/Barbearia/home.blade.php');
...
{
    $templatePath = resource_path('Templates/SalaoBeleza/home.blade.php');
    if (file_exists($templatePath)) {
        return file_get_contents($templatePath);
    }
    return '<!-- Template não encontrado -->';
